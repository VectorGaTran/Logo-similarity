...
for domain, img in logo_data.items():
    ...
    # Normalize and resize feature vector
    if len(hog_features) < fixed_size:
        hog_features = np.pad(hog_features, (0, fixed_size - len(hog_features)), mode='constant')
    else:
        hog_features = hog_features[:fixed_size]
    ...
    full_feature_vector = np.hstack((hog_features, hist))
...
clustering = AgglomerativeClustering(n_clusters=None, distance_threshold=30).fit(feature_vectors)
...
# Save output
with open("logo_groups_HOG.json", "w") as f:
    json.dump(clusters, f, indent=4)

print("Clustering complete. Results saved in 'logo_groups_HOG.json'.")
